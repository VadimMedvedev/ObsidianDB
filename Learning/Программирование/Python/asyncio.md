...
Затем перед каждой функцией, от которой мы будем требовать асинхронность, будем писать async (они становяться coroutine и могут быть остановлены и возобновлены):
``` python
async def print1():
	print(1)
```
...
Последние две строчки можно можно записать и как:
```python
	await asyncio.gather(task1, task2)
```
Эквивалентная запись. Более того в функцию gather можно передавать задания из списка (при помощи \*), а список можно генерировать динамически.
Или же всю функцию main можно записать так:
``` python
async def main():
...
```
В этом блоке функции можно запускать хоть при помощи циклов, условий и т.д. Суть в том, что все как либо запущенные в этом блоке кода функции будут пытаться выполниться одновременно.

Важно, что в асинхронных функциях, в которых нам важно в какой последовательности будут выполняться команды, мы используем просто ключевые слова await, которые будут говорить асинхронным функциям выше (которые вызвали эту функцию), что можно пока начать выполнять другую, пока эта ждёт ответа от команды.

!!! Для того чтобы использовать библиотеку asyncio с другими библиотеками, те должны быть тоже асинхронными!!! Не все библиотеки, к сожалению, такими являються. Но для многих библиотек есть их ассинхронные версии, перед ними часто ставится aio:
* request -> aiohttp (http запросы)
* telegrambotapi -> aiogram (создание телеграм бота)
* sqlite -> aiosqlite (подключение к БД)
